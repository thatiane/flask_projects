...
def delete_one():
    con = sql.connect("database.db")
    cur = con.cursor()
    # cur.execute("delete from students where name == '.....'")
    # cur.execute("delete from students where addr == '.....'")
    # cur.execute("delete from students where city == '.....'")
    # cur.execute("delete from students where Phone == 1234567890")
    con.commit()
    msg = "now one student has been deleted".title()
    return render_template("result.html", msg=msg)
