...
def homepage():
    ...
    headline_html = """<a href="{url}" target="_blank">YouTube video: ADELE-HELLO</a>&emsp;<a href="/videos/" target="_blank">videos&#8811;</a>""".format(url=youtube_url)
...
def videos(vid):
    ...
    if True == is_url_ok(youtube_url):
        headline_html = """<a href="{url}" target="_blank">YouTube video: {id}</a>""".format(url=youtube_url, id=vid)
